...
exports.run = async (client, message, args) => {
    ...
    for(var citer1=0;citer1<iter_1.length;citer1++){
        ...
        if(citer1==iter_1.length-1){
            ...
            userName=user_dt_spl[0];
            var date_topic = user_dt_spl.slice(1).join(" ").split("|");
            date=date_topic[0];
            topic=date_topic[1];
        }
        if(userName == 'me'){
    ...
    if(ms > 0){
        ...
        client.runReminder(createdReminder);
        let d=new Date(ms);
        ...
        if(user_list.length < 1) descText+= ' everyone in ';
        user_list.forEach(ul_item=>{
            descText+= ul_item.toString();
        });
        if(role_list.length > 0 && user_list.length > 0) descText+=' and everyone in ';
        ...
        let embed = new MessageEmbed()
            ...
            .addField("Date",`${zeroPad(d.getDay(),2)}/${zeroPad(d.getMonth(),2)}/${d.getFullYear()} at ${zeroPad(dHour,2)}:${zeroPad(d.getMinutes(),2)}:${zeroPad(d.getSeconds(),2)} ${am}`)
        ...
        message.channel.send(embed).then(async msg_react=>{
            ...
            msg_react.awaitReactions(cancelling_filter,{
                max: 1,
                time: Math.min(20000,Math.max((ms-Date.now()-2000),0)),
                errors: ['time']
            }).then(async collected=>{
                msg_react.reactions.removeAll();
                var count = await client.deleteReminder(createdReminder._id);
                if(count){
